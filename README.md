...
- Shows the text detected on the visible PDF page below the extraction result as an expanded single vertical column of coordinate-keyed fragments, with each fragment's unrounded registered X/Y key beside the text.
...
7. Check the expanded **検出したテキスト** area below the extraction result if you want to confirm all text fragments found on the visible page in one vertical column with unrounded placement X/Y points beside each text item.
