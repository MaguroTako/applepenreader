...
- Registers positioned text fragments in a coordinate map keyed by quantized X/Y placement points, then uses a `Set` of hit coordinate keys during outline extraction so duplicate text is not emitted.
- Shows the text detected on the visible PDF page below the extraction result as a single vertical column of coordinate-keyed fragments, with each fragment's registered X/Y key beside the text.
