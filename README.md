...
- Adds a **配線リスト** tab with a 4-column, 100-row wiring table for checking wiring entries, highlights the touched cell with a red border while restoring the previous cell border, offers **NO** and **NC** selectors in the **Connect** cells, and copies a tapped extraction-result row into the selected red-bordered cell.
...
4. Open the **配線リスト** tab when you want to view the 4-column, 100-row wiring table. Tap a wiring-list cell to mark its border red; tapping another cell restores the previous cell and marks the new one. Use the **Connect** cell selector when you want to choose **NO** or **NC**.
