...
- Adds a **配線リスト** tab with a 4-column, 100-row wiring table for checking wiring entries, highlights the touched cell with a red border while restoring the previous cell border, shows registered **Connect** strings as tappable tags on a single click/tap, allows keyboard entry in every wiring-list cell including **Connect** cells with a double click/tap, and copies a tapped extraction-result row into the selected red-bordered cell.
...
- PDF.js is embedded in `index.html`; no network access is required after checkout or download.
- iPad Safari is the primary target, but desktop browsers can be used with a mouse for quick checks.

## How to try it

1. Open `index.html` directly on an iPad, or serve this directory with a local web server, for example:
...
4. Open the **配線リスト** tab when you want to view the 4-column, 100-row wiring table. Tap a wiring-list cell to mark its border red; tapping another cell restores the previous cell and marks the new one. Double-tap any wiring-list cell when you want to type with the iPad software keyboard. Single-click/tap a **Connect** cell to show registered strings as tags and select one; double-click/tap the cell when you want to type another value with the iPad software keyboard.
...
PDF.js is embedded directly in `index.html` so the prototype can run offline after the HTML file is available on the device. The app disables the PDF.js worker so iPad Safari can load PDFs without a local web server or companion JavaScript files. This keeps the prototype small and easy to run without Xcode. For production use, consider adding page thumbnails or direct page-number input, supporting OCR for scanned PDFs, and smoothing or simplifying hand-drawn outlines.
