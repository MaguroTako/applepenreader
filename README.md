...
- Deduplicates positioned text fragments by quantized placement point while keeping the original text rectangle for outline hit testing.
- Shows the text detected on the visible PDF page below the extraction result as a single vertical column of detected fragments, with each fragment's deduplicated, quantized placement X/Y point beside the text.
